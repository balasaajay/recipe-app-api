...
# Create public subnet - Inbound and Outbound internet access
...
# add routes
resource "aws_route" "public_internet_accesss_a" {
  route_table_id         = aws_route_table.public_a.id
...
# For resources in availability zone b
...
resource "aws_route" "public_internet_accesss_b" {
  route_table_id         = aws_route_table.public_b.id
  destination_cidr_block = "0.0.0.0/0" # public internet
  gateway_id             = aws_internet_gateway.main.id
}
...
# Add private subnets - Outbound only/No Inbound traffic. example - db hosts
